...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.ait = await ArtichainToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.ait.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.ait.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.ait.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.ait.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.ait.owner()).valueOf(), carol);
    });

    it('should also work with MasterArt', async () => {
        ...
        this.chef = await MasterArt.new(this.ait.address, dev, minter, '1000', { from: alice });
        await this.ait.transferOwnership(this.chef.address, { from: alice });
